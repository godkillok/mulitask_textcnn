...
flags.DEFINE_integer("train_epoch", 10000,
                     "Number of (global) training steps to perform")
flags.DEFINE_string("data_dir", "/data/tanggp/text_cnns/data/",
                    "Directory containing the dataset")
flags.DEFINE_string("test_dir", " /data/tanggp/some_test/deeplearning-master/word_cnn/dbpedia_csv/test*",
                    "Directory containing the dataset")
...
if FLAGS.work_type!='':
    cluster = {'chief': ['10.0.23.57:2222'],
               'ps': ['10.0.23.57:2223','10.0.18.64:2223'],
               'worker': ['10.0.18.64:2222']}#10.0.18.64'10.0.18.57:2224',
    os.environ['TF_CONFIG'] = json.dumps(
              {'cluster': cluster,
               'task': {'type': FLAGS.work_type, 'index': FLAGS.task_id}})
...
def parse_exmp(serialized_example):
    feats = tf.parse_single_example(
        ...
        features={
            ...
            "seg_ment": tf.FixedLenFeature([FLAGS.sentence_max_len], tf.int64),
            "tfidf": tf.FixedLenFeature([FLAGS.sentence_max_len], tf.float32),
...
def my_model2(features, labels, mode, params):
...
def my_model(features, labels, mode, params):
    ...
    sentence = features['text']
    segment=features["seg_ment"]
    tfidf = features["tfidf"]
    segment=tf.one_hot(segment, depth=4, dtype=tf.float32)
    tfidf=tf.expand_dims(tfidf, -1)
    # Get word embeddings for each token in the sentence
